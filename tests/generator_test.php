 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_forumimproved_generator_testcase extends advanced_testcase {
    public function test_generator() {
        global $DB;

        $this->resetAfterTest(true);

        $this->assertEquals(0, $DB->count_records('forumimproved'));

        $course = $this->getDataGenerator()->create_course();

        /** @var mod_forumimproved_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_forumimproved');
        $this->assertInstanceOf('mod_forumimproved_generator', $generator);
        $this->assertEquals('forumimproved', $generator->get_modulename());

        $generator->create_instance(array('course'=>$course->id));
        $generator->create_instance(array('course'=>$course->id));
        $forum = $generator->create_instance(array('course'=>$course->id));
        $this->assertEquals(3, $DB->count_records('forumimproved'));

        $cm = get_coursemodule_from_instance('forumimproved', $forum->id);
        $this->assertEquals($forum->id, $cm->instance);
        $this->assertEquals('forumimproved', $cm->modname);
    ...
    public function test_create_discussion() {
        ...
        $forum = self::getDataGenerator()->create_module('forumimproved', $record);
        ...
        self::getDataGenerator()->get_plugin_generator('mod_forumimproved')->create_discussion($record);
        self::getDataGenerator()->get_plugin_generator('mod_forumimproved')->create_discussion($record);
        self::getDataGenerator()->get_plugin_generator('mod_forumimproved')->create_discussion($record);

        // Check the discussions were correctly created.
        $this->assertEquals(3, $DB->count_records_select('forumimproved_discussions', 'forum = :forum',
    ...
    public function test_create_post() {
        ...
        $discussion = self::getDataGenerator()->get_plugin_generator('mod_forumimproved')->create_discussion($record);

        // Add a bunch of replies, changing the userid.
        $record = new stdClass();
        $record->discussion = $discussion->id;
        $record->userid = $user2->id;
        self::getDataGenerator()->get_plugin_generator('mod_forumimproved')->create_post($record);
        ...
        $this->assertEquals(4, $DB->count_records_select('forumimproved_posts', 'discussion = :discussion',
    ...
    public function test_create_content() {
        ...
        $course = self::getDataGenerator()->create_course();
        $forum = self::getDataGenerator()->create_module('forumimproved', array('course' => $course));

        $generator = self::getDataGenerator()->get_plugin_generator('mod_forumimproved');
        ...
        $discussionrecords = $DB->get_records('forumimproved_discussions', array('forum' => $forum->id));
        $postrecords = $DB->get_records('forumimproved_posts');
        $postrecords2 = $DB->get_records('forumimproved_posts', array('discussion' => $post1->discussion));
