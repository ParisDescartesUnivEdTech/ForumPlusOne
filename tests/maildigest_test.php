...
class mod_forumplusone_maildigest_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        // $CFG->forumplusone_digestmailtimelast = 0;
        $this->initialconfig->digestmailtimelast = get_config('forumplusone', 'digestmailtimelast');
        ...
        // NOTE: we can't temporarilly set the global variable anymore now that the config is a plugin config
        // $CFG->forumplusone_digestmailtime = -1;
        $this->initialconfig->digestmailtime = get_config('forumplusone', 'digestmailtime');
        set_config('digestmailtime', -1, 'forumplusone');
    ...
    public function tearDown() {
        ...
        set_config('digestmailtimelast', $this->initialconfig->digestmailtimelast, 'forumplusone');
        set_config('digestmailtime', $this->initialconfig->digestmailtime, 'forumplusone');
    }
    ...
    protected function helper_setup_user_in_course() {
        ...
        $return->forums->forum1 = $this->getDataGenerator()->create_module('forumplusone', $record);
        $return->forumsids[] = $return->forums->forum1->id;

        $return->forums->forum2 = $this->getDataGenerator()->create_module('forumplusone', $record);
    ...
    protected function helper_force_digest_mail_times() {
        ...
        $digesttime = usergetmidnight(time(), $CFG->timezone) + (get_config('forumplusone', 'digestmailtime') * 3600) - (60 * 60);
        $DB->set_field('forumplusone_posts', 'modified', $digesttime, array('mailed' => 0));
        $DB->set_field('forumplusone_posts', 'created', $digesttime, array('mailed' => 0));
    }
    ...
    public function test_set_maildigest() {
        ...
        $currentsetting = $DB->get_record('forumplusone_digests', array(
            'forum' => $forum1->id,
            'userid' => $user->id,
        ));
        ...
        forumplusone_set_user_maildigest($forum1, 0, $user);
        $currentsetting = $DB->get_record('forumplusone_digests', array(
            'forum' => $forum1->id,
            'userid' => $user->id,
        ));
        ...
        forumplusone_set_user_maildigest($forum1, 1, $user);
        $currentsetting = $DB->get_record('forumplusone_digests', array(
            'forum' => $forum1->id,
            'userid' => $user->id,
        ));
        ...
        forumplusone_set_user_maildigest($forum1, 2, $user);
        $currentsetting = $DB->get_record('forumplusone_digests', array(
            'forum' => $forum1->id,
            'userid' => $user->id,
        ));
        ...
        forumplusone_set_user_maildigest($forum1, 42, $user);
    }
    ...
    public function test_get_user_digest_options_default() {
        ...
        $digestoptions = array(
            '0' => get_string('emaildigestoffshort', 'mod_forumplusone'),
            '1' => get_string('emaildigestcompleteshort', 'mod_forumplusone'),
            '2' => get_string('emaildigestsubjectsshort', 'mod_forumplusone'),
        );

        // The default settings is 0.
        $this->assertEquals(0, $user->maildigest);
        $options = forumplusone_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumplusone', $digestoptions[0]));

        // Update the setting to 1.
        $USER->maildigest = 1;
        $this->assertEquals(1, $USER->maildigest);
        $options = forumplusone_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumplusone', $digestoptions[1]));

        // Update the setting to 2.
        $USER->maildigest = 2;
        $this->assertEquals(2, $USER->maildigest);
        $options = forumplusone_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_forumplusone', $digestoptions[2]));
    }
    ...
    public function test_cron_profile_single_mails() {
        ...
        forumplusone_set_user_maildigest($forum2, -1, $user);
    ...
    public function test_cron_mixed_email_2() {
        ...
        forumplusone_set_user_maildigest($forum2, 0, $user);
    ...
    public function test_cron_forum_digest_email() {
        ...
        forumplusone_set_user_maildigest($forum2, 2, $user);
