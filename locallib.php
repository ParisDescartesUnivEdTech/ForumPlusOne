 ...
 * @author Mark Nielsen
 */
class forumplusone_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'forumplusone');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('forumplusone_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'forumplusone');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('forumplusone_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'forumplusone');
        }

        if (!$this->forum = $DB->get_record('forumplusone', array('id' => $this->discussion->forum))) {
            throw new portfolio_caller_exception('invalidforumid', 'forumplusone');
        }

        if (!$this->cm = get_coursemodule_from_instance('forumplusone', $this->forum->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumplusone', 'attachment', $this->post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumplusone', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = forumplusone_get_all_discussion_posts($this->discussion->id);
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_forumplusone', 'attachment', $post->id, 'timemodified', false);
                $embed  = $fs->get_area_files($this->modcontext->id, 'mod_forumplusone', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/forumplusone/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/forumplusone/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('forumdiscussion' . $this->discussionid,
                    get_string('discussion', 'forumplusone') . ': ' . $this->discussion->name, 'selection');
    ...
    /**
     * this is a very cut down version of what is in forumplusone_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_forumplusone', 'post', $post->id, $format);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' .  get_string('attachments', 'forumplusone') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/forumplusone:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/forumplusone:exportownpost', $context)));
        }
        return has_capability('mod/forumplusone:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'forumplusone');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class forumplusone_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    public function __construct($browser, $course, $cm, $context, $areas, $filearea) {
        ...
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_forumplusone';
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_forumplusone', $this->filearea, $record->itemid))
